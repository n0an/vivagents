...
export class CodexProvider implements CLIProvider {
  ...
  constructor(config: ProviderConfig, timeout: number) {
    ...
    if (this.isAvailable) {
      logger.info(`Codex CLI found at ${this.binaryPath}`);
  ...
  findBinary(customPath?: string | null): string | null {
    return findBinary('codex', customPath);
  }

  async enhance(params: EnhanceParams): Promise<string> {
    if (!this.binaryPath) {
      throw Object.assign(new Error('Codex CLI binary not found'), { code: 'BINARY_NOT_FOUND', status: 500 });
    }
